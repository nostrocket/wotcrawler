...
- New migration `0003` is **index-only**: add an index supporting the staleness scan over `last_fetched_at` (the existing `pubkeys_status_idx` partial index deliberately excludes `fetched`, which the scanner must re-enqueue). Keep the migration additive/idempotent per the established 0001/0002 conventions.
- FRESH-03 churn capture: **the churn columns already exist** — `pubkeys.last_changed_at`, `fetch_count`, and `change_count` shipped in migration 0001 and are already bumped by `apply_follow_list` (verified `follows.rs:120-135`). Phase 4 therefore does **not** add columns; it ensures the staleness-refresh path exercises that existing write (each re-fetch updates fetch_count, and a changed list bumps change_count + last_changed_at) and exposes the churn data. This grounds a future adaptive policy (FRESH-04, v2) without reprocessing. [Correction from research A3: original draft said "add new columns"; they pre-exist.]
